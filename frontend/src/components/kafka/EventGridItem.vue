<template>
  <div class="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-md transition-all duration-200 hover:border-blue-300 group">
    ...
    <div class="mb-3">
      <h4 class="text-sm font-semibold text-slate-900 group-hover:text-blue-900 transition-colors mb-1">
        {{ event.title }}
      </h4>
    ...
    <div class="space-y-2 mb-3">
      ...
      <div v-if="event.source" class="flex items-center gap-1 text-xs text-slate-500">
        <Database :size="12" />
        <span>{{ event.source }}</span>
      </div>
      
      <div v-if="event.shipName" class="flex items-center gap-1 text-xs text-slate-500">
        <Ship :size="12" />
        <span>{{ event.shipName }}</span>
      </div>
      
      <div v-if="event.containerId" class="flex items-center gap-1 text-xs text-slate-500">
        <Package :size="12" />
        <span>{{ event.containerId }}</span>
      </div>
    </div>

    <!-- Action Buttons -->
    <div class="flex items-center justify-end gap-1 pt-2 border-t border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity">
      <button 
        @click="$emit('view-event', event)"
        class="p-1.5 text-slate-400 hover:text-blue-600 transition-colors rounded"
        title="View Details"
      >
        <Eye :size="14" />
      </button>
      <button 
        ...
        class="p-1.5 text-slate-400 hover:text-red-600 transition-colors rounded"
        title="Delete Event"
      >
        <Trash2 :size="14" />
      </button>
